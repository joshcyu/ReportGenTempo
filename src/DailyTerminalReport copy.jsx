...
const DailyTerminalReport = () => {
  ...
  const fetchReport = async () => {
    ...
    const endpoint =
      endDate && dayjs(endDate).isBefore(cutoffDate.add(1,'day'))
        ? "http://10.10.10.61:5001/api/daily-terminal-report"
        : "http://10.10.10.61:5000/api/daily-terminal-report";
  ...
  const fetchDetailsForDate = async (rawDate) => {
    ...
    const res = await axios.get(
      "http://10.10.10.61:5000/api/daily-terminal-report/details",
      { params: { date: rawDate } }
    );
  ...
  const fetchTxnDetails = async (transID) => {
    ...
    if (!txnDetails[transID]) {
      const res = await axios.get(
        `http://10.10.10.61:5000/api/transactions/${transID}/details`
      );
                                ...
                                <td colSpan={2 + terminals.length + 6} className="p-0">
                                  ...
                                  >
                                    <table className="min-w-full bg-white">
                                      <thead className="bg-gray-200 text-gray-700">
                                        <tr>
                                          <th className="pl-20 py-2 text-left">RecordID</th>
                                          {[
                                      ...
                                      <tbody className="divide-y divide-gray-300">
                                        {txnDetails[txn.TransID]?.map(item => (
                                          <tr key={item.RecordID} className="bg-white hover:bg-gray-50">
                                            <td className="pl-20 py-2">{item.RecordID}</td>
